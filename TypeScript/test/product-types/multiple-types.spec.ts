...
describe('All items', () => {
  ...
  it.each([
    ...
    (item) => {
      ...
      expect(() => gildedRose.updateQuality()).toThrow(`Quality is out of bounds: ${item.quality}`);
    });
  ...
  it.each([
    [new Item('Standard Item', 'abc', 50)],
    [new Item('Aged Brie', 2, [])],
    [new Item('Backstage passes to a TAFKAL80ETC concert', 15, 'test')],
    [new Item('Conjured Mana Cake', {}, 5)]
  ])('should throw an error if quality or sellIn is NaN',
    (item) => {
      const gildedRose = new GildedRose([item]);
      expect(() => gildedRose.updateQuality()).toThrow(`Quality or sellIn is NaN`);
    });
